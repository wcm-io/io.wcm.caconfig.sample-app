...
# Build application
./build-deploy.sh build --maven.profiles=fast,aem66 --conga.environment=local-aem65 "$@"
if [ "$?" -ne "0" ]; then
  pause_message
  exit $?
fi
...
./build-deploy.sh deploy --maven.profiles=fast,aem66 --conga.node=aem-author --conga.environment=local-aem65 "$@" &

# Deploy to publish (in parallel)
./build-deploy.sh deploy --maven.profiles=fast,publish,aem66 --conga.node=aem-publish --conga.environment=local-aem65 "$@" &
